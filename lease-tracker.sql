    ...
    WITH
        ...
        leasing_request AS (
            SELECT
                ...
                IFNULL(wholesale_billing_status_c, '') AS wholesale_billing_status,
                ...
                CAST(
                    CONCAT(CAST(CAST(lease_start_date_c AS DATE) AS STRING), 'T', LEFT(CAST(lease_start_time_c AS STRING), 12)) AS DATETIME
                ) AS start_date_of_current_lease,
                CAST(
                    CONCAT(CAST(CAST(end_date_c AS DATE) AS STRING), 'T', LEFT(CAST(lease_end_time_c AS STRING), 12)) AS DATETIME
        ...
        leasing_request_user AS (
            ...
            FROM
                leasing_request lr
                LEFT JOIN user u ON lr.account_manager = u.user_id
        ),
        ...
        billing_data AS (
            ...
            FROM
                ...
                INNER JOIN invoice i ON lru.id = i.leasing_request
        ),
        ...
        final_output AS (
            SELECT
                ...
                (
                    DATE_DIFF(DATE_ADD(end_date_of_current_lease, INTERVAL 1 DAY), start_date_of_current_lease, DAY) / 365.25
    ...
    SELECT
        *,
        ...
        CASE
            ...
            AND wholesale_billing_status NOT IN ('Billed', 'Billed - Manually', 'Billed - SV', 'Billing Not Required')
