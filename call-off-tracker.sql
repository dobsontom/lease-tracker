    ...
    WITH
        leasing_request_call_off_block AS (
            SELECT
                ...
                lr.lsp_c AS lsp,
                lr.lsr_c AS lsr,
                IFNULL(lr.project_id_c, '') AS project_id,
                ...
                FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP(CONCAT(DATE(cob.cob_start_date_c), ' ', cob.cob_start_time_c))) AS start_date_time,
                FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP(CONCAT(DATE(cob.cob_end_date_c), ' ', cob.cob_end_time_c))) AS end_date_time,
