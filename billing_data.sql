CREATE OR REPLACE VIEW `inm-iar-data-warehouse-dev.lease_tracker.billing_data` AS (
    ...
    SELECT
        lru.ssp_number AS `SSP number`,
        lru.lsp AS `LSP`,
        lru.lsr AS `LSR`,
        lru.end_customer AS `End Customer`,
        lru.account_number AS `Account Number`,
        lru.land_aero_maritime AS `Land Aero Maritime`,
        i.wholesale_or_retail AS `Invoice Wholesale or Retail`,
        i.invoice_number AS `Invoice Number`,
        i.name AS `Invoice Name`,
        i.wholesale_credit_rebill AS `Invoice Wholesale Credit Rebill`,
        i.billing_source AS `Invoice Billing Source`,
        lru.service_type AS `Service Type`,
        lru.start_date_of_current_lease AS `Start Date of Current Lease`,
        lru.end_date_of_current_lease AS `End Date of Current Lease`,
        i.bill_period_start AS `Invoice Bill Period Start`,
        i.bill_period_end AS `Invoice Bill Period End`,
        i.billing_status AS `Invoice Billing Status`,
        i.billed_amount AS `Invoice Billed Amount`,
        i.created_date AS `Invoice Created Date`
    FROM
